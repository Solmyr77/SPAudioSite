         ...
         await this.settingsCollection.getList(1, 3).then((settings) => {
            settings.items.forEach((record) => {
               if (record.setting == "votesVisible") {
                  ...
                  console.log(this.songLimit.value);
               }
            });
         });
      },
...
<template>
   <div
      class="items-center flex min-h-screen flex-col bg-custom-bg-dark text-white"
   >
      <div
         class="grid grid-cols-12 bg-custom-custom-elevation-3 w-full text-center elevation-3 justify-center items-center"
      >
         <div
            class="flex elevation-4 items-center justify-center border-r-2 border-x-slate-400 h-full col-span-2"
         >
            <button
               @click="toggleSongLimit"
               class="text-white rounded-lg text-l p-1 mt-1 mb-1 active:bg-rose-950"
               :class="[
                  this.songLimit.value == 1
                     ? 'bg-rose-700 elevation-4 hover:bg-rose-500'
         ...
         <div
            class="flex items-center justify-center border-x-2 border-x-slate-400 h-full col-span-2"
         >
            <button
               @click="toggleVotesVisible"
               class="bg-emerald-600 elevation-4 hover:bg-emerald-500 text-white rounded-lg text-l p-1 mt-1 mb-1 active:bg-rose-950"
               :class="[
         ...
         <div
            class="flex items-center justify-center col-span-2 border-l-2 border-r-4 border-x-slate-400 h-full"
         >
            <button
               @click="setSongsPerPerson"
               class="w-2/5 bg-emerald-600 elevation-4 hover:bg-emerald-500 text-white rounded-lg text-l p-1 mt-1 mb-1 active:bg-rose-950"
            >
               Set requests per person
            </button>

            <div class="w-2/5 flex justify-center items-center">
               <input
                  class="text-black w-3/4 text-center"
                  type="number"
                  min="1"
                  max="999"
         ...
         <div class="col-span-3 flex justify-around items-center">
            ...
            <h1 class="text-xl">Version: E2.0</h1>
         </div>

         <div
            class="flex items-center justify-center border-l-4 border-r-2 border-x-slate-400 h-full"
         >
            <button
               @click="deleteAll"
         ...
         <div
            class="flex items-center justify-center border-x-2 border-x-slate-400 h-full"
         >
            <button
               @click="resetVotes"
               class="w-3/4 bg-rose-700 elevation-4 hover:bg-rose-500 text-white rounded-lg text-l p-1 mt-1 mb-1 active:bg-rose-950"
         ...
         <div
            class="flex items-center justify-center border-l-2 border-x-slate-400 h-full"
         >
            <button
               @click="clearUsers"
               class="w-3/4 bg-rose-700 elevation-4 hover:bg-rose-500 text-white rounded-lg text-l p-1 mt-1 mb-1 active:bg-rose-950"
      ...
      <div class="flex items-center m-0 p-0 w-full">
         <ul class="text-center w-full overflow-y-auto mb-4">
            <li class="" v-for="item in zenek" :key="item.id">
               <div
                  class="hover:bg-custom-custom-elevation-3 w-full grid grid-cols-36 pt-2 pb-2 gap-2 elevation-2"
               >
                  <span></span>
                  <button
                  ...
                  <span
                     v-if="item.ytlink == ''"
                     class="text-3xl col-span-27 overflow-hidden flex items-center justify-center"
                     ><a
                        target="_blank"
                        class="hover:text-gray-400"
                        :href="`https://www.google.com/search?q=${item.title}`"
                        >{{ item.title }}</a
                     ><br /><span class="text-red-400"
                        ><a class="hover:text-red-500" :href="item.ytlink">{{
                           item.ytlink
                        }}</a></span
                     ></span
                  >
                  <span v-else class="text-3xl col-span-27 overflow-hidden"
                     >{{ item.title }}<br /><span class="text-red-400"
                        ><a
                           target="_blank"
                  ...
                  <span
                     ...
                     class="justify-center items-center flex text-3xl overflow-hidden col-span-2"
                     >{{ item.votes }}</span
                  >
               </div>
               <span></span>
